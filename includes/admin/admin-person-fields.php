...
function ctc_person_title_text( $title ) {
	...
	if  ( 'ctc_person' == $screen->post_type ) {
		$title = __( 'Enter name here', 'churchthemes-framework' );
	}
...
function ctc_add_meta_box_person_details() {
	...
	$meta_box = array(
		...
		'title' 	=> _x( 'Person Details', 'meta box', 'churchthemes-framework' ),
		...
		'fields' => array(
			...
			*/
			...
			'_ctc_person_position' => array(
				'name'				=> __( 'Position', 'churchthemes-framework' ),
				'after_name'		=> '', // (Optional), (Required), etc.
				'after_input'		=> '', // text to show to right of input (fields: text, select, number, upload, url, date, time)
				'desc'				=> __( "Enter the person's position or title (e.g. Senior Pastor, Deacon, etc.)", 'churchthemes-framework' ),
				'type'				=> 'text', // text, textarea, checkbox, radio, select, number, upload, upload_textarea, url, date, time
				'checkbox_label'	=> '', //show text after checkbox
				'options'			=> array(), // array of keys/values for radio or select
				'upload_button'		=> '', // text for button that opens media frame
			...
			'_ctc_person_email' => array(
				'name'				=> _x( 'Email', 'location meta box', 'churchthemes-framework' ),
				'after_name'		=> '', // (Optional), (Required), etc.
				'after_input'		=> '', // text to show to right of input (fields: text, select, number, upload, url, date, time)
				'desc'				=> '',
				'type'				=> 'text', // text, textarea, checkbox, radio, select, number, upload, upload_textarea, url, date, time
			...
			'_ctc_person_urls' => array(
				'name'				=> __( 'URLs', 'churchthemes-framework' ),
				'after_name'		=> '', // (Optional), (Required), etc.
				'after_input'		=> '', // text to show to right of input (fields: text, select, number, upload, url, date, time)
				'desc'				=> '',
...
function ctc_person_columns( $columns ) {
	...
	$insert_array = array();
	$insert_array['ctc_person_thumbnail'] = __( 'Thumbnail', 'churchthemes-framework' );
	$columns = ctc_array_merge_after_key( $columns, $insert_array, 'cb' );

	// insert columns after title
	$insert_array = array();
	if ( ctc_field_supported( 'people', '_ctc_person_position' ) ) $insert_array['ctc_person_position'] = __( 'Position', 'churchthemes-framework' );
	if ( ctc_taxonomy_supported( 'people', 'ctc_person_group' ) ) $insert_array['ctc_person_group'] = _x( 'Groups', 'people column', 'churchthemes-framework' );
	$insert_array['ctc_person_order'] = _x( 'Order', 'sorting', 'churchthemes-framework' );
	$columns = ctc_array_merge_after_key( $columns, $insert_array, 'title' );

	//change "title" to "name"
	$columns['title'] = _x( 'Name', 'person', 'churchthemes-framework' );
